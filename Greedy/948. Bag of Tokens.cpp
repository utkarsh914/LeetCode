...
whenevr we are eligible to buy, we will buy
otherwise we will sell
If we aren't able to sell too, game ends

we keep updating the ans at every buy.
*/
...
public:
	...
	int bagOfTokensScore(vector<int>& A, int p) {
		...
		for (int i = 0, j = A.size() - 1; i <= j; ++i)
			if (p >= A[i]) p -= A[i], ++res;
			// sell one point to buy from last, and then
			// but from start, in just one step
			else if (res) p += A[j--] - A[i];
			else break;
		return res;
	}
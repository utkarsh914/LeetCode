...
bool search(vector<int>& n, int target) {
	...
	while (l <= r) {
		...
		while (n[l] == n[mid] && l != mid) l++;
		
		while (n[mid] == n[r] && r != mid) r--;

		// Checking if both target and n[mid] are on same side.
		if ((target < n[l] and n[mid] < n[l])
			or (target >= n[l] and n[mid] >= n[l])) {
			comparator = n[mid];
		}
		else {
			// Trying to figure out where n[mid] is and making comparator as -INF or INF
			if (target < n[l]) comparator = -INFINITY;
			else comparator = INFINITY;
		}

		if (target == comparator) return true;
		if (target > comparator) l = mid+1;
		else r = mid-1;

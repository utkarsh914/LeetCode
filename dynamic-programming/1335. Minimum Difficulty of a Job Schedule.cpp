...
public:
	int minDifficulty(vector<int>& jobs, int d) {
		...
		int ans = finish(jobs, 0, d);
		return ans < INT_MAX/2 ? ans : -1;
	}
	
	int finish(vector<int>& jobs, int start, int d) {
		if (d > jobs.size() - start) return INT_MAX / 2;
		if (start == jobs.size()) return 0;
		if (dp[start][d] != -1) return dp[start][d];
		int mx = INT_MIN, ans = INT_MAX / 2;
		for (int i = start; i < jobs.size(); i++) {
			mx = max(mx, jobs[i]);
			ans = min(ans, mx + finish(jobs, i+1, d-1));
		}
